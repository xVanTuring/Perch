...
struct MarkdownEngineNoteEditor: View {
    ...
    private var configuration: MarkdownEditorConfiguration {
        ...
        // 图片引用的 UUID 留在正文里(`![[名称|UUID]]`),不走引擎默认的「藏进文本
        // 属性、显示成 ![[名称]]」。属性只在那段文字还能解析成 embed 时才活着 ——
        // 删一个括号、整段改名、把这行复制到别处,UUID 就没了,而且找不回来:图片
        // 还在 Core Data 里,但没有任何东西叫得出它。放在正文里就看得见、选得中、
        // 跟着备份走。代价是光标停在图片上时那行源码比较长。
        config.imageEmbed.keepsIDInSource = true
